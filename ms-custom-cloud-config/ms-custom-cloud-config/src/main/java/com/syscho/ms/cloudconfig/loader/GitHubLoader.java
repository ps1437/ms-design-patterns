...
import com.syscho.ms.cloudconfig.watcher.GithubWatcher;
...
@Slf4j
@Service
public class GitHubFileLoader extends FileLoader {

    private final ConfigProperties configProperties;
    private final ConfigCacheManager cacheManager;
    private final Path repoDirectory;

    public GitHubFileLoader(ConfigCacheManager cacheManager, ConfigProperties configProperties) {
        super(cacheManager);
...
//                }
    ...
    @PostConstruct
    void addWatcher() {
        log.info("Initializing GitHubWatcher...");
        new GithubWatcher(cacheManager, configProperties, repoDirectory).watch(repoDirectory, cacheManager);
    }
}
